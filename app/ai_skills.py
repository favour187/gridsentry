...
def build_offline_report(alert, reading: dict | None) -> dict:
    ...
    if reading:
        lines.append(f"Current telemetry: {reading['kw']} kW at {reading['voltage']} V (baseline {reading.get('baseline_kw')} kW) on {reading['transformer']}.")
    if ev:
    ...
    return {'provider': 'offline', 'alert_id': alert.id, 'report': '\n\n'.join(lines), 'action': action, 'confidence': 'high' if alert.severity == 'critical' else 'medium'}
...
def refine_with_llm(alert, reading: dict | None) -> dict | None:
    ...
    try:
        r = httpx.post(f'{base}/chat/completions', headers={'Authorization': f'Bearer {key}'}, json={'model': model, 'messages': [{'role': 'system', 'content': 'You are a grid-operations investigator. Using ONLY the facts in the draft report, rewrite it as a concise incident brief for a field supervisor: What happened / Evidence / Likely cause / Field action. No new numbers. Max 160 words. Plain text.'}, {'role': 'user', 'content': offline['report']}], 'temperature': 0.2, 'max_tokens': 900}, timeout=8)
        r.raise_for_status()
        text = r.json()['choices'][0]['message']['content'].strip()
        return {'provider': f'llm:{model}', 'alert_id': alert.id, 'report': text, 'action': offline['action'], 'confidence': offline['confidence']}
    except Exception:
        return None
