...
import random
NOMINAL_V = 230.0
NETWORK = {'FEEDER-A': {'tf': ['TX-A1', 'TX-A2']}, 'FEEDER-B': {'tf': ['TX-B1']}}
METER_SPECS = [('M-101', 'Sabon Gari market row', 'FEEDER-A', 'TX-A1', 'shop', 0.85, 120, 150), ('M-102', 'Welding workshop 3', 'FEEDER-A', 'TX-A1', 'industrial', 1.6, 210, 150), ('M-103', 'Amina cold store', 'FEEDER-A', 'TX-A1', 'commerce', 1.1, 300, 150), ('M-104', 'Block 7 flats', 'FEEDER-A', 'TX-A1', 'residential', 0.45, 390, 150), ('M-105', 'Barber shop 12', 'FEEDER-A', 'TX-A1', 'shop', 0.22, 480, 150), ('M-106', 'Rice mill line B', 'FEEDER-A', 'TX-A2', 'industrial', 2.1, 120, 300), ('M-107', 'Chemist + clinic', 'FEEDER-A', 'TX-A2', 'health', 0.55, 210, 300), ('M-108', 'Mallam tea stall', 'FEEDER-A', 'TX-A2', 'shop', 0.12, 300, 300), ('M-109', 'Hostel block C', 'FEEDER-A', 'TX-A2', 'residential', 0.7, 390, 300), ('M-110', 'Phone repair kiosk', 'FEEDER-A', 'TX-A2', 'shop', 0.1, 480, 300), ('M-201', 'Grain dryer 1', 'FEEDER-B', 'TX-B1', 'industrial', 1.8, 160, 450), ('M-202', 'Primary school', 'FEEDER-B', 'TX-B1', 'public', 0.4, 250, 450), ('M-203', 'Tailor collective', 'FEEDER-B', 'TX-B1', 'shop', 0.35, 340, 450), ('M-204', 'Water pumping point', 'FEEDER-B', 'TX-B1', 'utility', 1.2, 430, 450), ('M-205', 'Musa duplex', 'FEEDER-B', 'TX-B1', 'residential', 0.5, 520, 450), ('M-206', 'Night market strip', 'FEEDER-B', 'TX-B1', 'commerce', 0.95, 610, 450), ('M-207', 'Borehole 2', 'FEEDER-B', 'TX-B1', 'utility', 0.9, 700, 450), ('M-208', 'Church annex', 'FEEDER-B', 'TX-B1', 'public', 0.3, 790, 450)]
...
def hour_shape(hour: int, kind: str) -> float:
    ...
    if kind == 'residential':
        peak = [0.5, 0.4, 0.35, 0.3, 0.35, 0.5, 0.7, 0.8, 0.7, 0.6, 0.6, 0.65, 0.7, 0.7, 0.7, 0.75, 0.9, 1.15, 1.35, 1.4, 1.3, 1.1, 0.9, 0.65][hour]
    elif kind in ('industrial', 'utility'):
        peak = [0.6, 0.6, 0.6, 0.6, 0.6, 0.7, 0.9, 1.1, 1.25, 1.3, 1.3, 1.25, 1.2, 1.25, 1.3, 1.3, 1.2, 1.0, 0.8, 0.7, 0.65, 0.6, 0.6, 0.6][hour]
    elif kind in ('shop', 'commerce'):
        peak = [0.3, 0.25, 0.2, 0.2, 0.2, 0.3, 0.5, 0.7, 0.9, 1.1, 1.2, 1.25, 1.2, 1.15, 1.2, 1.25, 1.3, 1.25, 1.1, 0.9, 0.7, 0.55, 0.45, 0.35][hour]
    else:
        peak = [0.5, 0.45, 0.4, 0.4, 0.4, 0.5, 0.6, 0.8, 0.9, 0.9, 0.85, 0.8, 0.8, 0.85, 0.9, 0.9, 0.85, 0.8, 0.85, 0.9, 0.85, 0.75, 0.65, 0.55][hour]
    return base * peak
...
def synth_meter(spec, now: dt.datetime, rng: random.Random, scenario: dict | None):
    ...
    reading = {'meter_id': mid, 'ts': now.isoformat() + 'Z', 'voltage': round(v, 1), 'current': round(cur, 2), 'kw': round(kw, 3), 'kwh_export': kwh_export, 'reverse_events': reverse_events, 'freq_hz': round(freq, 2), 'pf': round(pf, 2), 'phases': [round(p, 1) for p in phases], 'phase_current': [round(c, 2) for c in phase_cur], 'flags': flags}
    return reading
