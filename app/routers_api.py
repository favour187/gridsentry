...
from .engine import evaluate, fleet_health, meter_risk, risk_from_open
from .ai_skills import investigate
router = APIRouter(prefix='/api')
SCENARIO_KINDS = ('bypass', 'sag', 'imbalance', 'offline', 'tamper', 'neutral', 'freq', 'overload', 'outage')
...
def seed(db: Session):
    ...
    for mid, name, feeder, tx, kind, base, x, y in sim.METER_SPECS:
        db.add(Meter(id=mid, name=name, feeder=feeder, transformer=tx, kind=kind, baseline_kwh_h=base, token=sim.token_for(mid), x=x, y=y))
    db.commit()
...
def _alert_payload(a: Alert) -> dict:
    return {'id': a.id, 'kind': a.kind, 'severity': a.severity, 'meter_id': a.meter_id, 'scope': a.scope, 'title': a.title, 'evidence': json.loads(a.evidence or '{}'), 'status': a.status, 'created_at': a.created_at.isoformat() + 'Z', 'updated_at': a.updated_at.isoformat() + 'Z'}
...
@router.get('/health')
def health():
    return {'status': 'ok', 'service': 'gridsentry', 'version': '2.0', 'time': dt.datetime.utcnow().isoformat() + 'Z'}
...
def _fresh_alerts(db: Session, snapshot, now):
    ...
    for a in fresh:
        ...
        if row:
            ...
            row = Alert(key=a['key'], kind=a['kind'], severity=a['severity'], meter_id=a['meter_id'], scope=a['scope'], title=a['title'], evidence=json.dumps(a['evidence']))
            db.add(row)
    db.commit()
    return fresh
...
def live(db: Session=Depends(get_db)):
    ...
    risks, health_score = risk_from_open(alerts, list(SPEC_META))
    at_risk = sorted(({'meter_id': mid, 'score': v['score'], 'reasons': v['reasons']} for mid, v in risks.items() if v['score'] > 0), key=lambda x: -x['score'])
    meters = []
    for r in snapshot:
        risk = risks.get(r['meter_id'], {'score': 0, 'reasons': []})
        meters.append({**{k: r[k] for k in ('meter_id', 'name', 'feeder', 'transformer', 'kind', 'x', 'y', 'voltage', 'current', 'kw', 'baseline_kw', 'reverse_events', 'phase_current', 'phases', 'flags', 'freq_hz', 'pf')}, 'status': _status_for(r), 'risk': risk['score'], 'risk_reasons': risk['reasons'], 'ts': r['ts']})
    freqs = [r['freq_hz'] for r in snapshot if r.get('freq_hz')]
    return {'now': now.isoformat() + 'Z', 'meters': meters, 'alerts': alerts[:80], 'scenarios': scen, 'at_risk': at_risk[:8], 'kpis': {'delivered_kw': round(delivered, 2), 'expected_kw': round(expected, 2), 'loss_pct': round((1 - delivered / expected) * 100, 1) if expected > 0 else 0.0, 'online': len(snapshot), 'total': len(SPEC_META), 'alerts_open': len(alerts), 'critical': sum((1 for a in alerts if a['severity'] == 'critical')), 'health': health_score, 'grid_freq_hz': round(sum(freqs) / len(freqs), 2) if freqs else None}}
...
def stats(db: Session=Depends(get_db)):
    ...
    now = dt.datetime.utcnow()
    for a in all_rows:
        k = by_kind.setdefault(a.kind, {'kind': a.kind, 'open': 0, 'resolved': 0, 'critical': 0, 'warning': 0})
    ...
    total = len(all_rows)
    return {'total': total, 'by_status': by_status, 'by_severity': by_severity, 'by_kind': sorted(by_kind.values(), key=lambda x: -(x['open'] + x['resolved'])), 'resolution_rate_pct': round(100 * resolved_count / total, 1) if total else 0.0, 'mttr_minutes': round(sum(mttr_minutes) / len(mttr_minutes), 1) if mttr_minutes else None, 'trend_24h': trend}
...
def incidents_csv(db: Session=Depends(get_db)):
    ...
    buf = io.StringIO()
    w = csv.writer(buf)
    w.writerow(['id', 'kind', 'severity', 'meter_or_scope', 'status', 'title', 'evidence', 'created_at_utc', 'updated_at_utc'])
    for a in db.query(Alert).order_by(Alert.created_at.desc()).limit(5000).all():
        w.writerow([a.id, a.kind, a.severity, a.meter_id or a.scope, a.status, a.title, a.evidence, a.created_at.isoformat() + 'Z', a.updated_at.isoformat() + 'Z'])
    return Response(content=buf.getvalue(), media_type='text/csv', headers={'Content-Disposition': 'attachment; filename=gridsentry-incidents.csv'})

@router.get('/devices')
def devices(db: Session=Depends(get_db)):
    seed(db)
    return {'devices': [{'meter_id': m.id, 'name': m.name, 'token': m.token, 'feeder': m.feeder, 'transformer': m.transformer} for m in db.query(Meter).order_by(Meter.id).all()]}
...
def meter_detail(meter_id: str, db: Session=Depends(get_db)):
    ...
    return {'meter_id': meter_id, 'name': spec[1], 'feeder': spec[2], 'transformer': spec[3], 'kind': spec[4], 'history': sim.history_series(spec)}
...
def list_scenarios(db: Session=Depends(get_db)):
    ...
    db.commit()
    return {'active': [{'id': s.id, 'kind': s.kind, 'meter_id': s.meter_id, 'started_at': s.started_at.isoformat() + 'Z'} for s in db.query(Scenario).filter(Scenario.active == True).all()]}
...
def ingest(meter_id: str, body: IngestIn, authorization: str=Header(default=''), db: Session=Depends(get_db)):
    ...
    now = dt.datetime.utcnow()
    db.add(Reading(meter_id=meter_id, ts=now, voltage=body.voltage, current=body.current, kw=body.kw, kwh_export=body.kwh_export, reverse_events=body.reverse_events, phases=json.dumps(body.phases)))
    spec = next((s for s in sim.METER_SPECS if s[0] == meter_id))
    reading = {'meter_id': meter_id, 'name': spec[1], 'feeder': spec[2], 'transformer': spec[3], 'kw': body.kw, 'voltage': body.voltage, 'baseline_kw': spec[5], 'reverse_events': body.reverse_events, 'freq_hz': body.freq_hz, 'pf': body.pf, 'phase_current': body.phase_current, 'flags': {'cover_open': body.cover_open}}
    fresh = evaluate([reading], [meter_id], now, spec_meta=SPEC_META)
    raised = []
    for a in fresh:
        row = db.query(Alert).filter(Alert.key == a['key']).first()
        if not row:
            row = Alert(key=a['key'], kind=a['kind'], severity=a['severity'], meter_id=meter_id, scope=a['scope'], title=a['title'], evidence=json.dumps(a['evidence']))
            db.add(row)
